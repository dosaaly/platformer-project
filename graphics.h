...
void draw_parallax_background() {
    ...
    float initial_offset = -(PlayerManager::get_instance().get_player().get_pos().x * PARALLAX_PLAYER_SCROLLING_SPEED + game_frame * PARALLAX_IDLE_SCROLLING_SPEED);
...
void draw_game_overlay() {
    ...
    // Hearts
    for (int i = 0; i < PlayerManager::get_instance().get_player().get_lives(); i++) {
    ...
    // Score
    Vector2 score_dimensions = MeasureTextEx(menu_font, std::to_string(PlayerManager::get_instance().get_total_score()).c_str(), ICON_SIZE, 2.0f);
    Vector2 score_position = {GetRenderWidth() - score_dimensions.x - ICON_SIZE, slight_vertical_offset};
    DrawTextEx(menu_font, std::to_string(PlayerManager::get_instance().get_total_score()).c_str(), score_position, ICON_SIZE, 2.0f, WHITE);
    ...
    for (size_t row = 0; row < current_level.rows; ++row) {
        for (size_t column = 0; column < current_level.columns; ++column) {
            ...
            Vector2 pos = {
                    ...
                    (static_cast<float>(column) - PlayerManager::get_instance().get_player().get_pos().x) * cell_size + horizontal_shift,
...
void draw_player() {
    ...
    Vector2 pos = {
            horizontal_shift,
            PlayerManager::get_instance().get_player().get_pos().y * cell_size
    };

    // Pick an appropriate sprite for the player
    if (game_state == GAME_STATE) {
        if (!PlayerManager::get_instance().get_player().is_on_ground()) {
            draw_image((PlayerManager::get_instance().get_player().is_looking_forward() ? player_jump_forward_image : player_jump_backwards_image), pos, cell_size);
        }
        else if (PlayerManager::get_instance().get_player().is_moving()) {
            draw_sprite((PlayerManager::get_instance().get_player().is_looking_forward() ? player_walk_forward_sprite : player_walk_backwards_sprite), pos, cell_size);
            PlayerManager::get_instance().get_mutable_player().set_moving(false);
        }
        else {
            draw_image((PlayerManager::get_instance().get_player().is_looking_forward() ? player_stand_forward_image : player_stand_backwards_image), pos, cell_size);
        }
    }
    else {
...
void draw_enemies() {
    ...
    for (auto &enemy : EnemiesController::get_instance().get_enemies()) {
        ...
        Vector2 pos = {
                (enemy.get_pos().x - PlayerManager::get_instance().get_player().get_pos().x) * cell_size + horizontal_shift,
